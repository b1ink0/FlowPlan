...
function SideNavbar() {
  ...
  return (
    <div className={`${!showSideNavbar ? "-translate-x-full" : ""} z-10 transition-all duration-200 w-[280px] grow-0 h-full absolute left-0 top-0 bg-gray-900 text-gray-200 flex flex-col justify-center items-center gap-1 border-r-2 border-slate-700`}>
      <div className={`w-6 h-12 rounded-r-full absolute right-0 translate-x-[24px] bg-slate-700 flex justify-center items-center p-1 cursor-pointer`} onClick={() => setShowSideNavbar(prev => !prev)}><BackIcon/></div>
